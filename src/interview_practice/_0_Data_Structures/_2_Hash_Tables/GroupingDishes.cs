        ...
        string[][] groupingDishes(string[][] dishes) {
            ...
            foreach(var dish in dishes) {
                ...
                for(int i = 1; i < dish.Length; i++) {
                    if (!builder.ContainsKey(dish[i])) {
                        builder.Add(dish[i], new List<string>());
                    }//if (!builder.ContainsKey(dish[i])) {
                    builder[dish[i]].Add(food);
                }//for(int i = 1; i < dish.Length; i++) {
            }//foreach(var dish in dishes) {
            var keyList = builder.Keys.ToList();
            keyList.Sort(StringComparer.Ordinal);
            foreach(var item in keyList) {
                if (builder[item].Distinct().Count() >= 2) {
                    builder[item].Sort(StringComparer.Ordinal);
                    List<string> storeList = new List<string>();
                    storeList.Add(item);
                    storeList.AddRange(builder[item].Distinct());
                    output.Add(storeList.ToArray());
                }//if (builder[item].Distinct().Count() >= 2) {
            }//foreach(var item in keyList) {
